...
STOCKS_PATH = ROOT_DIR / "stocks.txt"

OUTPUT_PATH = (
    SCRIPT_DIR
    / f"market_5min_{EVENT_DATE.isoformat()}_{TRADING_DAYS_TO_FETCH}d.csv"
)
...
def load_symbols() -> list[str]:
    if not STOCKS_PATH.exists():
        raise FileNotFoundError(
            f"stocks.txt not found: {STOCKS_PATH}"
        )
    ...
    if not symbols:
        raise RuntimeError("stocks.txt contains no symbols")
...
def main() -> int:
    print("=" * 70)
    print("IRAN EVENT MARKET DATA FETCH")
    print("=" * 70)
